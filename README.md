...
go to the root of project run the command docker-compose up -d
run the npm i
run tin command to create table in the db using npm run migrate
then run the seeder to create role and admin user by default in the code using npm run seed and then l

    find some curl as well for example
...
curl --location 'http://localhost:3000/roles' \
--header 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6ImFkbWluQGV4YW1wbGUuY29tIiwic3ViIjo5LCJyb2xlcyI6WyJhZG1pbiJdLCJpYXQiOjE3NTE3ODQ4NzMsImV4cCI6MTc1MTc4ODQ3M30.wieh9XAwaeK2XpqqYXQJHjxYQfRcZCUdVohpzNaysf0'
